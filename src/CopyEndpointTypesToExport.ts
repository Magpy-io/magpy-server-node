...
async function RemovedJoiSchemasFromTypes(outputPathAbsolute: string) {
  const tsfiles = await glob(
    join(outputPathAbsolute, "EndpointsApi", "/**/*.ts"),
    {
      absolute: true,
      nodir: true,
    }
  ...
  for (let tsfile of tsfiles) {
    ...
    const sI = parsed.imports.find((e) => e.libraryName == "joi")
      ?.start as number;
    const eI = parsed.imports.find((e) => e.libraryName == "joi")
      ?.end as number;

    const s = parsed.declarations.find((e) => e.name == "RequestSchema")
      ?.start as number;
    const e = parsed.declarations.find((e) => e.name == "RequestSchema")
      ?.end as number;

    const parsedFile =
      fileStr.substring(0, sI) +
      fileStr.substring(eI, s) +
      fileStr.substring(e);
