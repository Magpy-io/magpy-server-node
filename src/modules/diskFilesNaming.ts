...
async function createServerImageName<T extends createServerImageNameInputType>(photo: T) {
...
async function createServerImageThumbnailName<T extends createServerImageNameInputType>(
  photo: T,
) {
  const dirPath = GetStorageFolderPath();
  const nameParsed = path.parse(photo.name);
...
async function createServerImageCompressedName<T extends createServerImageNameInputType>(
  photo: T,
) {
  const dirPath = GetStorageFolderPath();
  const nameParsed = path.parse(photo.name);
