...
async function verifyFileDeclarations(filePath: string) {
  ...
  const responseDataType = parsed.declarations.find(
    (e) => e.name == "ResponseData"
  );
  const RequestShemaObject = parsed.declarations.find(
    (e) => e.name == "RequestSchema"
  );
  ...
  if (
    !responseDataType ||
    !(responseDataType instanceof TypeAliasDeclaration)
  ) {
    throw new Error(
      "ResponseData type must be exported in file '" + filePath + "'"
    );
  }

  if (
    !RequestShemaObject ||
    !(RequestShemaObject instanceof VariableDeclaration)
  ) {
    throw new Error(
  ...
  try {
    const endpointString = fileStr.substring(
      endpointName.start as number,
      endpointName.end
    );
